...
class SyDENet(nn.Module):
    def __init__(self, args):
        ...
        # 为单目特征添加SE模块
        mono_channels = mono_model_configs[args.encoder]['out_channels']
        #self.se_mono = LayerNormSEBlock(256, reduction=16)#K
        self.se_mono = FG_RCA(256, up_scale=1)#K
    ...
    def forward(self, image1, image2, iters=12, test_mode=False):
        ...
        #对单目特征应用SE注意力
        features_mono_left = [
            self.se_mono(features_mono_left[0]),
            self.se_mono(features_mono_left[1]),
            self.se_mono(features_mono_left[2]),
            self.se_mono(features_mono_left[3])
        ]
