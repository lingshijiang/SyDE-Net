...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # 处理损坏的PNG文件
            print(f"损坏的图像文件: {self.image_list[index]}, 错误: {str(e)}")
            # 尝试使用不同的索引
            alt_index = (index + 1) % len(self.image_list)
            return self.__getitem__(alt_index)
        except Exception as e:
            print(f"读取图像时出错: {self.image_list[index]}, 错误: {str(e)}")
            alt_index = (index + 1) % len(self.image_list)
            return self.__getitem__(alt_index)
...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/mnt/e/MonSter-main/core/data2/cjd/StereoDatasets/sceneflow', dstype='frames_finalpass', things_test=False):
